...
ConstantPairwiseForce::ConstantPairwiseForce(double force, double cutoff_length)
    : force_{force}, cutoff_length_{cutoff_length}
{}

physics::ForceContribution ConstantPairwiseForce::operator() (Eigen::Vector4d separation) const
{
    ...
    if (norm < cutoff_length_)
    {
        // The potential grows linearly with distance:
        force_contribution.potential = force_ * (norm - cutoff_length_);
...
double ConstantPairwiseForce::cutoff_length() const
{return cutoff_length_;}

double ConstantPairwiseForce::square_cutoff_length() const
